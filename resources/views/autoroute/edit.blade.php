...
<div class="container-fluid">
	...
	@endif
	<form id="partybillForm" method="POST" action="{{ url('/') }}/{{$route}}/{{$table->id}}" target="">
	{{ csrf_field() }}
	@method('PUT')
			...
			@foreach($columns as $item)
				...
				<div class="col-sm-6">
					<div class="form-group">
		                ...
		                @else
		                <input type="text" class="form-control @if(isset($key) && array_key_exists($item, $key)) {{$key[$item]}} @endif" id="{{$item}}" name="{{$item}}" value="{{$table->$item}}">
		                @endif
					</div>
				</div>
				@endif
